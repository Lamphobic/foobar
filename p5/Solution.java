...
public class Solution {
    public static String solution(String x, String y) {
        BigInteger max = new BigInteger(x);
        BigInteger min = new BigInteger(y);
		BigInteger gen = new BigInteger("0");
		BigInteger one = BigInteger.ONE;
		BigInteger zero = BigInteger.ZERO;
		while(true){
			if(max.compareTo(one) == 0 && min.compareTo(one) == 0){
				return ("" + gen);
			}
			else if (max.compareTo(one) < 0 || min.compareTo(one) < 0 || max.compareTo(min) == 0){
				return "impossible";
			}
			if(max.compareTo(min) < 0){ //make min the max val if greater than max
				BigInteger tmp = max;
				max = min;
				min = tmp;
			}
			if(max.mod(min).compareTo(zero) == 0){
				gen = gen.add(max.divide(min)).subtract(one);
				max = max.mod(min).add(min);
			} else {
				gen = gen.add(max.divide(min));
				max = max.mod(min);
			}
		}
    }
