...
public class Solution {
    public static int solution(int n) {
		...
		totals[0] = 1;
		for(int i = 1; i <= n; i++){ //size of lookback
			for(int j = n; j >= i; j--){ //where to start lookback 
				totals[j] += totals[j-i]; //append a new step of size i to all steps at (j-i) and add those steps to j
			}
		}
		return totals[n]-1;
    }
